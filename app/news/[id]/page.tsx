...
// Dynamically import the renderer (client-only, skips SSR for voting auth)
const NewsContentRenderer = dynamic(() => import("@/components/news-content-renderer"), {
  ssr: false,
  loading: () => <div className="h-32 animate-pulse rounded-2xl bg-black/5" />,
});
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(0,0,0,0.06),_transparent_30%),linear-gradient(180deg,_#ffffff_0%,_#f7f7f5_52%,_#efefec_100%)] flex flex-col text-black">
      ...
      <main className="flex-grow w-full max-w-4xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        <article className="bg-white p-6 sm:p-12 rounded-[2.5rem] shadow-[0_24px_80px_rgba(15,23,42,0.08)] border border-black/5">
          ...
          {/* Cover Image */}
          <div className="relative w-full h-[350px] sm:h-[500px] mb-10 overflow-hidden rounded-[2rem] bg-black/5">
            <Image
          ...
          <div className="mb-10 max-w-3xl mx-auto">
            <p className="text-xl sm:text-2xl leading-relaxed font-medium text-black/90">
              {post.description}
            </p>
          </div>
          ...
          {(post.content ?? "").trim() ? (
            <div className="max-w-3xl mx-auto">
              <NewsContentRenderer content={post.content ?? ""} postId={post.id} />
            </div>
          ) : (
            <p className="max-w-3xl mx-auto border-t border-black/10 pt-6 leading-relaxed text-black/45">
